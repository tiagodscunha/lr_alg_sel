...
run_experiment <- function(type){
  ...
  #IR meta-problem
  A_result <- rank_evaluation(original,targets,perf_values,goal)
  B_result <- rank_evaluation(ranking,targets,perf_values,goal)
  C_result <- rank_evaluation(pairwise,targets,perf_values,goal)
  D_result <- rank_evaluation(ratio,targets,perf_values,goal)
...
save(IR, RP, file="results/base_graphics_landmarkers.Rda")


#bound é variavel - fixar ao maximo?
