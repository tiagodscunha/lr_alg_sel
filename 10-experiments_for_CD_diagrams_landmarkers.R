...
run_experiment <- function(type){
  ...
  #IR meta-problem
  A_result <- rank_evaluation(original,targets,"AB")
  B_result <- rank_evaluation(ranking,targets,"RK")
  C_result <- rank_evaluation(pairwise,targets,"PW")
  D_result <- rank_evaluation(ratio,targets,"RT")
...
save(IR, RP, file="detailed_meta_results_landmarkers.Rda")
