...
run_experiment <- function(type,metric){
  ...
  #IR meta-problem
  A_result <- rank_evaluation(B,targets)
  B_result <- rank_evaluation(C,targets)
  C_result <- rank_evaluation(D,targets)
  D_result <- rank_evaluation(E,targets)
  
  alternative_table <- data.frame(datasets=c("B","C","D","E"),
                                  avg = c(
...
save(NDCG, AUC, NMAE, RMSE, file="results/regular_metatarget_new_metafeatures_graphics.Rda")
