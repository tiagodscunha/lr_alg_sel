...
run_experiment <- function(type,metric){
  ...
  #IR meta-problem
  A_result <- rank_evaluation(A,targets)
  B_result <- rank_evaluation(B,targets)
  C_result <- rank_evaluation(C,targets)
  D_result <- rank_evaluation(D,targets)
  
  alternative_table <- data.frame(datasets=c("SM","SL","GR","UN"),
                                  avg = c(
...
save(NDCG, AUC, NMAE, RMSE, file="results/regular_metatarget_graphics.Rda")
